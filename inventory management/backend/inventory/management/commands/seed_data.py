...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if not User.objects.filter(email='manager@cse.edu').exists():
            User.objects.create_user(
                email='manager@cse.edu',
                name='Manager User',
                password='manager123',
                role='manager'
            )
        ...
        if not User.objects.filter(email='viewer@cse.edu').exists():
            User.objects.create_user(
                email='viewer@cse.edu',
                name='Viewer User',
                password='viewer123',
                role='viewer'
            )
            self.stdout.write(self.style.SUCCESS('✓ Viewer user created'))
        ...
        self.stdout.write('  Manager: manager@cse.edu / manager123')
        self.stdout.write('  Viewer:  viewer@cse.edu  / viewer123')
